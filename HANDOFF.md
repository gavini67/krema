...
- [ ] **Phase 0 — owner activation is deliberately paused.** Production setup and end-to-end tests passed on 2026-09-13, but the owner asked to keep accounts off the public site until further testing.
  - [x] **Brevo account** — free tier. SMTP key named `Supabase Auth`, **expires 2027-08-13**. Diary that: when it lapses, every password reset silently fails.
  - [x] **DNS at GoDaddy** — 6 of Brevo's 7 records were already present and correct. `_dmarc` was deliberately left on the stricter existing `p=quarantine` rather than Brevo's suggested `p=none`: DMARC passes on DKIM alignment alone, and only one DMARC record per domain is valid. Verified with `dig`; Brevo's own "Verify records" then passed all 7 green.
  - [x] **Supabase SMTP** — `smtp-relay.brevo.com:587`, login `b45a12001@smtp-brevo.com`, sender `noreply@kremadesserthaus.com`, min interval 60s.
  - [x] **Auth email templates** — Confirm signup + Reset Password pasted from `docs/email-templates/`, byte-verified against source after a page reload. Both use `{{ .Token }}`.
  - [x] **Turnstile** — enabled in Supabase Auth with the private Secret Key; the customer and staff production widgets both reached Success. The private key remains outside the repository.
  - [x] **Customer CSV backup + account migration** — exported 50 customer rows to `~/Downloads/customers_rows.csv`, then applied `docs/migrations/2026-09-13-customer-accounts.sql` live on 2026-09-13. Verification: 50 customers remain, 0 linked cards, `user_id` + `ON DELETE SET NULL` FK exist, both lookup signatures return 0 rows, anon cannot call `claim_card`/`get_my_card`/`unlink_card`, authenticated can call all three.
  - [ ] **Public email signup is OFF during the production hold.** Re-enable “Allow new users to sign up” in Supabase only when the account UI is released again. Confirm email remains enabled.
  - [x] **Frozen Auth settings verified:** minimum password length 6 and No required characters.
  - [x] **Production customer test passed:** signup, branded OTP email delivery to Gmail inbox, card claim, logout/login, forgot-PIN email, PIN reset, old-PIN rejection, and new-PIN login. Test card: `KREMA-0585`; owner test cards remain excluded from staff reporting.
  - [x] **Staff-side production test passed:** customer and staff remained signed in in separate mobile tabs; staff unlink disconnected the card without deleting stamps; the signed-in customer then re-linked `KREMA-0585` successfully.
...
- [ ] **Phase 3 — customer accounts are built and verified, but held from public release.** The complete tested version is preserved on `feature/customer-accounts-ready`. On `main`, `CUSTOMER_ACCOUNTS_ENABLED = false`, the account controls are hidden, and Supabase public signup is disabled. Migration: `docs/migrations/2026-09-13-customer-accounts.sql`.
...
- [ ] **Phase 4 — newsletter.** Wait until the customer-account production hold is resolved.
...
- [x] **"Rewards" links + club section** on the main site — header nav, mobile drawer and footer all link out; `#club` section sits after Reviews with a full 20-slot card, and the hero has a ticket teaser.
      ⚠️ **The club section duplicates the mechanic in marketing copy.** `CLUB_GOAL` / `CLUB_TIERS` near the `Club` component in `index.html` MUST match `krema_tiers()` in `supabase-setup.sql`. Change the mechanic → change both, or the homepage advertises a promo the app doesn't deliver. (The Claude Design prototype still shows the retired 11-stamp version; don't copy its numbers.)
...
- **Never move large HTML/text through a browser-automation JS-eval path as base64.** On 2026-08-04, pasting an email template into Supabase via a base64 blob was **silently truncated ~1100 chars** with no error — something in the transit layer mangles long base64-looking strings. Pass a normal JSON-escaped string literal instead, and verify **structural counts** (`<tr>`/`<td>`/`<table>`, and the `{{ .Token }}` occurrence count) not just byte length — length alone can look plausible while the markup is broken. Same run: setting the value via the Monaco editor API (`editor.setValue`) rather than simulated keystrokes avoids the editor's auto-closing brackets corrupting the HTML.
