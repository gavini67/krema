...
- [ ] **Phase 0 — owner activation. The code is prepared; production setup and end-to-end testing are still pending.**
  ...
  - [ ] **Turnstile** — the public production Site Key is embedded in the customer and staff pages. The private Secret Key belongs only in Supabase → Auth → Attack Protection; never put it in a page or this repository. Do not enable CAPTCHA until the CAPTCHA-capable pages are deployed, or staff can be locked out.
  ...
  - [ ] Next: deploy the CAPTCHA-capable pages; configure Supabase CAPTCHA with the private Secret Key; then enable public email signup.
  - [ ] Confirm the frozen Auth settings remain **minimum password length 6** and **No required characters**, then test a throwaway signup, OTP, claim, logout/login, recovery, staff-session isolation, and staff unlink. Do not mark live SQL, CAPTCHA, public signup, or end-to-end account testing complete until each has been performed.
...
- [ ] **Phase 3 — customer accounts.** SQL is live and verified; page deployment and end-to-end activation remain pending. Code is prepared on `feature/customer-accounts`; migration: `docs/migrations/2026-09-13-customer-accounts.sql`.
